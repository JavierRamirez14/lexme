...
programs blocks and their history, feeds proposed citations -- clean, corrupt or
mis-anchored -- and asserts the single verdict and the code-hydrated anchor.
"""
...
EVIDENCE = (
    EvidenceBlock(NORM_ID, "a9"),
    EvidenceBlock(NORM_ID, "a10"),
    EvidenceBlock(NORM_ID, "a11"),
)


def test_exact_citation_verifies_directly(lau_corpus: FakeCorpusReader) -> None:
    citation = ProposedCitation(block_id="a10", text="libremente estipulen las partes")
...
def test_one_changed_word_is_repaired_by_snap_to_the_real_text(
    ...
    citation = ProposedCitation(
        block_id="a10", text="La renta sera la que libremente estipularan las partes"
    )
...
def test_invented_citation_is_discarded_and_never_shown(lau_corpus: FakeCorpusReader) -> None:
    citation = ProposedCitation(
        block_id="a10", text="el inquilino podrá subarrendar sin límite alguno"
    )
...
def test_citation_to_a_block_outside_the_evidence_is_discarded(
    ...
    citation = ProposedCitation(block_id="a99", text="cualquier texto")
...
def test_valid_ellipsis_verifies(lau_corpus: FakeCorpusReader) -> None:
    citation = ProposedCitation(
        block_id="a10", text="La renta será la que […] estipulen las partes"
    )
...
def test_reordered_ellipsis_segments_do_not_verify(lau_corpus: FakeCorpusReader) -> None:
    citation = ProposedCitation(
        block_id="a10", text="estipulen las partes […] La renta será la que"
    )
...
def test_overlapping_ellipsis_segments_do_not_verify(lau_corpus: FakeCorpusReader) -> None:
    citation = ProposedCitation(
        block_id="a10", text="La renta será la que libremente […] libremente estipulen las partes"
    )
...
def test_too_short_ellipsis_segments_are_discarded(lau_corpus: FakeCorpusReader) -> None:
    citation = ProposedCitation(block_id="a10", text="La […] renta […] partes")
...
def test_verification_resolves_against_the_point_in_time_redaction(
    ...
    citation = ProposedCitation(block_id="a9", text="será de cinco años")
...
def test_misanchored_citation_is_re_anchored_to_the_single_matching_block(
    ...
    citation = ProposedCitation(
        block_id="a10", text="El arrendador podrá elevar la renta conforme a lo pactado"
    )

    [result] = verify_citations([citation], EVIDENCE, BEFORE_REFORM, lau_corpus)

    assert result.verdict is CitationVerdict.REPAIRED_ANCHOR
    assert result.block_id == "a11"
...
def test_ambiguous_re_anchor_across_several_blocks_is_discarded(
    ...
    evidence = (
        EvidenceBlock(NORM_ID, "a10"),
        EvidenceBlock(NORM_ID, "a12"),
        EvidenceBlock(NORM_ID, "a13"),
    )
    citation = ProposedCitation(block_id="a10", text=shared)
...
def test_each_result_carries_exactly_one_verdict_covering_all_four(
    ...
    citations = [
        ProposedCitation(block_id="a10", text=DIRECT_QUOTE),
        ProposedCitation(block_id="a10", text=SNAP_QUOTE),
        ProposedCitation(block_id="a10", text=REANCHOR_QUOTE),
        ProposedCitation(block_id="a10", text=INVENTED_QUOTE),
    ]
...
def test_corrupt_citations_from_the_llm_seam_are_discarded(
    ...
    synthesizer = FakeLlmClient(
        {
            "mode1_synthesis": [
                ProposedCitation(block_id="a10", text="texto fabricado por el modelo"),
                ProposedCitation(block_id="a404", text="bloque que nadie recuperó"),
            ]
        }
    )
